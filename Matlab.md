...
1. 绘制基础数据点（如散点或连线）；
2. 在每个数据点的 x 方向和 / 或 y 方向添加误差线，误差线的长度由输入的 “误差值” 决定；
3. 通过误差线的长短直观反映数据的不确定性（误差线越长，数据波动或测量误差越大）。

```matlab
% 1. 仅 y 方向误差（最常用）
errorbar(x, y, yerr)          % x: x轴坐标，y: y轴坐标，yerr: y方向误差值
errorbar(y, yerr)             % 省略x，默认x为1,2,...（数据索引）

% 2. x 方向误差（较少用）
errorbar(x, y, xerr, 'x')     % 'x'指定仅x方向误差（xerr为x方向误差值）

% 3. x 和 y 双方向误差
errorbar(x, y, xerr, yerr)    % xerr: x方向误差，yerr: y方向误差

% 4. 自定义误差线和数据点样式
errorbar(x, y, yerr, LineSpec)  % LineSpec指定数据点/连线样式（如颜色、标记）
errorbar(..., Name, Value)      % 属性-值对自定义误差线样式（如线宽、颜色）

% 5. 返回句柄（用于后续修改属性）
h = errorbar(...)              % h为误差线对象句柄
```

**参数解析：**

- **`x, y`**：数据点的坐标向量（长度相同），`x` 为横坐标，`y` 为纵坐标（核心数据）。
- **`yerr`**：y 方向误差值（最常用），可以是：
  - **标量**：所有数据点的 y 误差相同（如 `yerr=0.5` 表示每个点的 y 误差均为 0.5）；
  - **向量**：与 `y` 长度相同，每个元素对应一个数据点的 y 误差（如 `yerr=[0.3, 0.5, 0.4]`）；
  - **2×n 矩阵**：第一行表示每个点的 “下误差”（`y - 下误差`），第二行表示 “上误差”（`y + 上误差`），用于不对称误差（如 `yerr=[[0.2;0.3],[0.4;0.5]]` 表示第一个点下误差 0.2、上误差 0.3）。
- **`xerr`**：x 方向误差值，格式与 `yerr` 相同（标量 / 向量 / 2×n 矩阵），用于展示 x 坐标的不确定性。
- **`LineSpec`**：线条和标记样式字符串（与 `plot` 函数一致），如 `'ro-'` 表示红色圆圈标记 + 实线连接数据点。
- **属性 - 值对**：自定义误差线细节，常用属性包括：
  - `'LineWidth'`：数据连线的线宽；
  - `'MarkerSize'`：数据点标记的大小；
  - `'CapSize'`：误差线末端 “帽子” 的长度（默认 3，增大更易看清）；
  - `'Color'`：数据连线和误差线的颜色；
